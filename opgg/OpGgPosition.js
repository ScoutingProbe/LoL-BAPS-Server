...
function OpGgPosition(position, file){
...
OpGgPosition.prototype.main = async function(){
	let league = await readFile(path.resolve("opgg", `${this.file}.json`), "utf-8")
  ...
  let p = new Promise((resolve, reject)=>{
  	https.get(url, (res) => {
        ...
        if(counters.length == 1) {
          console.log(url)
          console.log(`OpGgPosition#main(${this.position} ${this.file} ${championName})`)
        	league.counters = []
        } else {
        ...
        await writeFile(path.resolve("opgg", `${this.file}.json`), JSON.stringify(league))
        resolve()
  		})
